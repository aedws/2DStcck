...
- 로그인 계정의 순자산이 `$1B` 이상이면 계정당 비상장 회사 하나를 설립할 수 있습니다.
- 설립 시 순자산의 20%를 현금으로 출자해 영구 소각하며, 회사 가치는 순자산 랭킹에
  다시 합산하지 않습니다.
- 이후 5거래일마다 해당 회차 시작 순자산의 5%를 추가 출자·소각하거나, 기존
  발행주식의 10%를 NPC 시장에 신주로 배정해 창업주 지분 희석을 선택합니다.
- 출자 거절 시 운영이 정지되고, 재개하려면 새 자본 확충 요구를 처리해야 합니다.
- 회사 성과는 최대 500 프레스티지로 반영됩니다. 출자 2회·자본 확충 4회·창업주
  지분 50%·회사 프레스티지 300을 충족하면 기존 종목 요청 경로로 IPO 심사를
  신청할 수 있습니다.
- 승인·정적 배포 전에는 거래할 수 없으며, 플레이어 간 전역 주식 원장은 사용하지
  않습니다.
...
기업의 화면 분류는 11개 상위 섹터로 통일합니다:
`방산·치안`, `산업재`, `반도체`, `소비재·서비스`, `식품·외식`, `채권`,
`금융`, `헬스케어`, `미디어·콘텐츠`, `기술`, `에너지·인프라`.
CSV에는 더 구체적인 `industry`를 입력하면 import 스크립트가 상위 섹터와 기존
시장 사건 태그를 함께 생성합니다. ETF·지수·선물·전략상품은 경제 섹터와 별도의
상품 분류로 표시됩니다.
...
| `industry` | ✅ | 세부 산업 — 상위 섹터·시장 사건 태그 자동 결정 | `방산` |
