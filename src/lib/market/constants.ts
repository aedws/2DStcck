 ...
 *  v39: 즉시 상장 '에덴 오트쿠튀르(EDEN)'·'엘리시아 파마(ELYS)'와 전용 사건 추가 —
 *  종목·파생상품 구성이 바뀌어 전체 체크포인트를 재생성한다.
 ...
 *  v42: 커버드콜 추종률·분배율 밸런스 조정 — 시세 산출이 바뀌어 재생성한다.
 *  v43: 나스닥 커버드콜 연 분배율 20%로 하향 — 시세 산출이 바뀌어 재생성한다.
 *  v44: 단일 종목 커버드콜 연 분배율 60%로 하향 — 시세 산출이 바뀌어 재생성한다.
 *  v45: 마카샤 연구소·몬텔리 캐피탈·히스클리프 리츠운용 3종 예약 상장(7/28~29)
 *  — 종목 구성이 바뀌어 전체 체크포인트를 재생성한다.
 *  v49: 캬롯 농장(CROT)의 수확량 호재 편향·장기 드리프트를 낮추고 일일 하락
 *  방어선을 -3%에서 -8%로 완화한다. 가격 경로 변경으로 전체 재생성한다.
 *  v50: 플레이어 회사 밀리테크 인터내셔널 아머먼츠(MILITC)와 전용 수주·안전성
 *  사건을 7/31 예약 상장한다. 종목 구성 변경으로 전체 재생성한다.
 *  v51: 이스마엘 해운(ISML)을 8/1 예약 상장하고, 공모가 하회 시 배당을 멈춰
 ...
 *  v57: 플레이어 회사 IPO 2종 '이가 증권(LEE)'·'Omni Robotics Industry(ORI)'을
 *  8/5 예약 상장한다. 비정상적으로 큰 누적 소각 장부가치는 시세 왜곡을 막기 위해
 *  공모가에 상한을 적용했다. 종목·파생상품 구성이 바뀌어 전체 체크포인트를 재생성한다. */
export const MARKET_SIM_VERSION = 57;
/**
