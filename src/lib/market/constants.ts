 ...
 *  전체 리플레이를 강제한다. */
export const MARKET_SIM_VERSION = 21;
/**
...
/**
 * 경제 상한(센트). 자바스크립트 정수 정밀도 한계(2^53 ≈ $90.07조)를 넘으면
 * 순자산·현금·주문 금액이 부정확해져 랭킹 갱신·매수가 조용히 실패한다. 그 한참
 * 아래인 $1조로 상한을 둬, 정상 플레이엔 절대 닿지 않으면서 어떤 값도 안전
 * 정수 범위(곱셈 여유 포함)에 머물게 한다. 순자산 표시·현금 적립·주문 금액에 적용.
 */
export const MONEY_CAP = 100_000_000_000_000;
