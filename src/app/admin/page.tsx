...
export default function AdminPage() {
  ...
  const refresh = useCallback(async () => {
    setRows(await listStockRequests());
  }, []);
  ...
  async function setStatus(id: string, status: StockRequestStatus) {
    setSavingId(id);
    const ok = await updateStockRequest(id, { status });
    if (ok) {
      setRows((prev) =>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">🛠️ 종목 추가 요청</h1>
          <p className="mt-1 text-sm text-[var(--muted)]">
            총 {rows.length}건 · 대기 {pendingCount}건
          </p>
        </div>
        <button
          type="button"
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[11px] text-[var(--muted)]">
                    @{r.game_id} · {new Date(r.created_at).toLocaleString("ko-KR")}{" "}
                    · {formatPrice(r.cost_paid)}
                  </p>
                </div>
                <span
                  className={`shrink-0 rounded-lg px-2 py-1 text-[11px] font-semibold ${STATUS_STYLE[r.status]}`}
                >
