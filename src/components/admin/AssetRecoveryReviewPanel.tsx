...
export function AssetRecoveryReviewPanel() {
  ...
  async function reject(row: AssetRecoveryRequestRow) {
    setSavingId(row.id);
    const result = await rejectAssetRecoveryRequest(
      row.id,
      resolutions[row.id]?.trim() ?? "",
    );
    setMessage(result.message);
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-sm font-semibold text-amber-300">확인 후 지급 원칙</p>
        <p className="mt-1 text-xs leading-relaxed text-[var(--muted)]">
          신고 금액은 자동 지급하지 않습니다. 서버 체크포인트·거래 원장·백업과 대조한
          정수 센트 금액만 먼저 검증하고, 별도 지급 버튼으로 1회 지급합니다.
        </p>
      </div>
      {message && (
                ...
                {row.status === "under_review" && (
                  <div className="mt-3 grid gap-2">
                    ...
                    <textarea
                      ...
                      placeholder="서버 스냅샷 시각·원장 행·계산식 등 검증 근거"
                      rows={3}
                ...
                {!closed && (
                  <div className="mt-3 grid gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        type="button"
                        disabled={savingId === row.id || (resolutions[row.id]?.trim().length ?? 0) < 10}
                        onClick={() => reject(row)}
                        className="rounded-lg bg-rose-500 px-3 py-2 text-xs font-semibold text-white disabled:opacity-40"
                      >
                        근거 불충분 · 회신
                      </button>
                    </div>
