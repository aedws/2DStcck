...
assert.equal(companies.length, 67);
assert.deepEqual(
...
assert.equal(instrumentCounts.get("company"), 67);
assert.equal(instrumentCounts.get("etf"), 8);
assert.equal(instrumentCounts.get("strategy"), 296);
...
assert.equal(strategyCounts.get("레버리지"), 77);
assert.equal(strategyCounts.get("인버스"), 77);
assert.equal(strategyCounts.get("곱버스"), 77);
assert.equal(strategyCounts.get("커버드콜"), 65);
